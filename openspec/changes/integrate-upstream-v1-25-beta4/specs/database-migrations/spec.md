...
Database upgrades MUST converge to one migration head from the pre-integration fork and upstream heads, preserving token-vending settings, API-key account ranks, forced-usage flags and continuation settings. Deployed migration revision identities MUST remain valid.
