...
## 1. Settings
- [x] 1.1 Add `account_token_vending_authority_base_url` / `_shared_secret` / `_access_token_skew_seconds` (env prefix `CODEX_LB_`).
- [x] 1.2 Normalize the authority URL (strip/rstrip); model-validate that when it is set the URL is `https://` with a hostname and the shared secret is present.
...
## 3. Follower gate (`AuthManager.ensure_fresh`)
- [x] 3.1 If `account_token_vending_authority_base_url` is set, vend and return an account with a re-encrypted access token; never rotate, never write refresh/id tokens, never persist. Gate sits ahead of the singleflight so all rotation paths are covered.

## 4. Authority endpoint
- [x] 4.1 `ProxyService.vend_access_token` — resolve account by `chatgpt_account_id` (fallback `account_id`), refresh via the normal singleflighted path, return only access token + expiry + identity.
- [x] 4.2 `POST /internal/bridge/oauth-token` — verify HMAC signature, 503 if unconfigured, 403 on bad/expired signature, 404 on unknown account, else the vend response.
