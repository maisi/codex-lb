...
@pytest.mark.asyncio
async def test_targeted_warmup_requires_accounts_write(
    app_instance: FastAPI, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    principal = _principal_without(Permission.ACCOUNTS_WRITE)
    assert principal.has(Permission.SECURITY_WRITE)
    _use_principal(app_instance, monkeypatch, principal)
    _assert_permission_required(await async_client.post("/api/accounts/missing/warmup"), Permission.ACCOUNTS_WRITE)

    _use_principal(app_instance, monkeypatch, _principal_with_only(accounts__write=Scope.ALL))
