...
@pytest.mark.asyncio
@pytest.mark.parametrize("path", ["/backend-api/codex/responses", "/v1/responses"])
async def test_http_bridge_quota_catalog_refresh_keeps_required_owner_out_of_policy_conflict(
    async_client, app_instance, monkeypatch, path
):
    import app.core.openai.model_refresh_scheduler as model_scheduler
    from app.core.openai.model_registry import get_model_registry

    _install_bridge_settings(monkeypatch, enabled=True)
    registry = get_model_registry()
    template = registry.get_models_with_fallback()["gpt-5.4"]
    fetched = []

    async def fetch(candidates, encryptor):
        del encryptor
        fetched.extend(account.id for account in candidates)
        model = replace(template, slug="gpt-6-astra", available_in_plans=frozenset({candidates[0].plan_type}))
        return model_scheduler._FetchResult(
            models=[model],
            account_models={account.id: (account.plan_type, [model]) for account in candidates},
        )

    monkeypatch.setattr(model_scheduler, "_fetch_with_failover", fetch)
    scheduler = model_scheduler.ModelRefreshScheduler(interval_seconds=60, enabled=True)
    owner_id = await _import_account(async_client, "catalog_bridge_owner", "bridge-owner@example.test")
    owner = await _get_account(owner_id)
    assert await scheduler._refresh_as_leader()
    upstream = _ClosingBridgeUpstreamWebSocket("resp_catalog_bridge")
    connected = []
    ...
    async def connect(headers, access_token, account_id, **kwargs):
        ...
        assert account_id == owner.chatgpt_account_id
        return upstream

    monkeypatch.setattr(proxy_module.ProxyService, "_ensure_fresh_with_budget", ensure_fresh)
    monkeypatch.setattr(proxy_module, "connect_responses_websocket", connect)
    headers = {"session_id": "catalog-bridge-session"}
    body = {"model": "gpt-6-astra", "instructions": "Continue", "input": "start", "stream": True}
    first = await _collect_sse_events(async_client, path, json_body=body, headers=headers)
    response_id = first[-1]["response"]["id"]
    sibling_id = await _import_account(
        async_client, "catalog_bridge_sibling", "bridge-sibling@example.test", plan_type="business"
    )
    async with SessionLocal() as session:
        await session.execute(
            update(Account)
            .where(Account.id == owner_id)
            .values(status=AccountStatus.QUOTA_EXCEEDED, reset_at=int(time.time()) + 3600, blocked_at=int(time.time()))
        )
        await session.commit()
    fetched.clear()
    assert await scheduler._refresh_as_leader()
    assert fetched == [sibling_id]
    response = await async_client.post(
        path, json={**body, "input": "continue", "previous_response_id": response_id}, headers=headers
    )
    assert response.status_code == 502, response.text
    assert response.json()["error"]["code"] == "previous_response_owner_unavailable"
    assert len(upstream.sent_text) == 1
    assert connected == [owner.chatgpt_account_id]
    assert registry.account_ids_for_model("gpt-6-astra") == frozenset({owner_id, sibling_id})
