...
async def test_api_key_cache_clamps_survive_fold_and_raw_pruning(db_setup):
    await _seed()
    async with SessionLocal() as session:
        await session.execute(update(RequestLog).values(input_tokens=10, cached_input_tokens=25))
        await session.execute(
            update(RequestLog).where(RequestLog.api_key_id == "key").values(input_tokens=-5, cached_input_tokens=-2)
        )
        await session.commit()
    expected, options = await _snapshot()
    assert {entry["api_key_id"] for entry in options["api_keys"]} == {"key", ""}
    entries = expected["by_api_key"]
    assert entries
    assert any(entry["cached_input_tokens"] > 0 for entry in entries)
    assert all(0 <= entry["cached_input_tokens"] <= entry["total_input_tokens"] for entry in entries)
    await _fold(END)
    actual, actual_options = await _snapshot()
    assert actual["by_api_key"] == entries
