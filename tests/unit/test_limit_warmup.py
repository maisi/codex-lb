...
# A weekly deadline far enough past the epoch that deriving a window start from
# it stays representable on every platform.
_WEEKLY_RESET_AT = 30 * 24 * 60 * 60
_SAMPLED_AT = datetime(2026, 6, 1, 12, 0, 0)
...
def _usage(
    ...
    *,
    ...
    if recorded_at is None:
        # Epoch arithmetic rather than datetime.fromtimestamp(): these synthetic
        # reset_at values put a long window's start before 1970, which the
        # platform converter rejects on Windows. The value produced is identical.
        recorded_at = _EPOCH + timedelta(seconds=reset_at - window_minutes * 60)
    return UsageHistory(
...
class FakeWarmupRepo:
    ...
    async def try_create_attempt(
        ...
        *,
        ...
        if require_no_prior_attempt and any(row.account_id == account_id for row in self.rows):
            return None
        if any(
...
async def test_streaming_limit_warmup_sender_rechecks_account_after_auth(
    ...
    async def ensure_fresh(target: Account, *, background: bool = False) -> Account:
        # Warm-up is a background pass, so borrowed accounts must not be vended here.
...
@pytest.mark.asyncio
async def test_unused_already_free_quota_warms_on_first_eligible_refresh() -> None:
    repo = FakeWarmupRepo()
    sender = FakeSender()
    service = LimitWarmupService(repo, FakeRequestLogsRepo(), sender=sender)
    account = _account()
    account.plan_type = "free"
    refresh_started_at = datetime(2026, 9, 8, 12, 0, tzinfo=timezone.utc).replace(tzinfo=None)
    ...
    await service.run_after_usage_refresh(
        ...
        before_secondary={
            account.id: _usage(
                ...
                window="monthly",
                recorded_at=refresh_started_at - timedelta(minutes=1),
            )
        },
        after_primary={},
        after_secondary={
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("sample_age_seconds", "used_percent", "prior_attempt", "previous_plan_type"),
    [
        (-1, 0.0, False, "free"),
        (0, 1.0, False, "free"),
        (0, 0.0, True, "free"),
    ...
    ids=["stale-sample", "partially-used", "prior-attempt", "unknown-prior-plan"],
)
async def test_initial_free_quota_candidate_rejects_unsafe_evidence(
    ...
    if prior_attempt:
        await repo.try_create_attempt(
            ...
            window="primary",
            reset_at=monthly_reset_at - 1,
            transition_key="test-prior-attempt",
            model="gpt-5.1-codex-mini",
            attempted_at=refresh_started_at - timedelta(days=1),
        )
    ...
    assert sender.calls == []
    assert len(repo.rows) == int(prior_attempt)


@pytest.mark.asyncio
async def test_initial_free_quota_warmup_does_not_repeat_when_reset_at_slides() -> None:
    repo = FakeWarmupRepo()
    sender = FakeSender()
    service = LimitWarmupService(repo, FakeRequestLogsRepo(), sender=sender)
    account = _account()
    account.plan_type = "free"
    first_refresh_at = datetime(2026, 9, 8, 12, 0, tzinfo=timezone.utc).replace(tzinfo=None)
    first_reset_at = int(first_refresh_at.replace(tzinfo=timezone.utc).timestamp()) + 43_200 * 60
    first_sample = _usage(
        account.id,
        used_percent=0,
        reset_at=first_reset_at,
        window="monthly",
        recorded_at=first_refresh_at,
    )
    ...
    await service.run_after_usage_refresh(
        ...
        before_secondary={},
        after_primary={},
        after_secondary={account.id: first_sample},
        previous_plan_types={account.id: "free"},
        refresh_started_at=first_refresh_at,
    )

    second_refresh_at = first_refresh_at + timedelta(minutes=1)
    await service.run_after_usage_refresh(
        accounts=[account],
        settings=_settings(limit_warmup_windows="secondary"),
        before_primary={},
        before_secondary={account.id: first_sample},
        after_primary={},
        after_secondary={
            account.id: _usage(
                account.id,
                used_percent=0,
                reset_at=first_reset_at + 60,
                window="monthly",
                recorded_at=second_refresh_at,
            )
        },
        previous_plan_types={account.id: "free"},
        refresh_started_at=second_refresh_at,
    )

    assert sender.calls == [(account.id, "gpt-5.1-codex-mini")]
    assert [(row.window, row.reset_at) for row in repo.rows] == [("monthly", first_reset_at)]


@pytest.mark.asyncio
async def test_primary_attempt_created_in_same_refresh_closes_initial_free_quota_path() -> None:
    repo = FakeWarmupRepo()
    sender = FakeSender()
    service = LimitWarmupService(repo, FakeRequestLogsRepo(), sender=sender)
    account = _account()
...
@pytest.mark.asyncio
async def test_skipped_primary_attempt_created_in_same_refresh_closes_initial_free_quota_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
    ...
    await service.run_after_usage_refresh(
        accounts=[account],
        settings=_settings(limit_warmup_windows="both", limit_warmup_model="auto"),
        before_primary={account.id: _usage(account.id, used_percent=100, reset_at=1000)},
        before_secondary={},
        after_primary={account.id: _usage(account.id, used_percent=0, reset_at=2000)},
        after_secondary={
    ...
    await service.run_after_usage_refresh(
        ...
        before_secondary={
            account.id: _usage(
                ...
                usage_id=10,
                used_percent=100,
                reset_at=_WEEKLY_RESET_AT,
                window="secondary",
                recorded_at=_SAMPLED_AT,
            )
        },
        after_primary={},
        after_secondary={
            account.id: _usage(
                ...
                usage_id=11,
                used_percent=0,
                # Upstream replenished the allowance and pulled the deadline back.
                reset_at=_WEEKLY_RESET_AT - 500,
                window="secondary",
                recorded_at=_SAMPLED_AT + timedelta(minutes=1),
            )
        },
    )
...
async def test_unplanned_weekly_reset_with_same_deadline_is_distinct_transition() -> None:
    ...
    await service.run_after_usage_refresh(
        ...
        after_secondary={
            account.id: _usage(
                ...
                usage_id=21,
                used_percent=0,
                # Replenished with the deadline left exactly where it was.
                reset_at=_WEEKLY_RESET_AT,
                window="secondary",
                recorded_at=_SAMPLED_AT + timedelta(minutes=1),
            )
    ...
    assert [(row.reset_at, row.transition_key) for row in repo.rows] == [
        (_WEEKLY_RESET_AT, "usage-history:10"),
        (_WEEKLY_RESET_AT, "usage-history:21"),
    ]
