...
class ReportsRepository:
    ...
    async def aggregate_by_api_key(
        ...
        source = report_source(
            self._session, [("api_key_cache", start_date, end_date)], account_ids, model, useragent_group, api_key_ids
        )
        result = await self._session.execute(
            select(
                source.c.api_key_id,
                ...
                ApiKey.key_prefix,
                func.coalesce(func.sum(source.c.request_count), 0).label("request_count"),
                func.coalesce(func.sum(source.c.input_tokens), 0).label("total_input_tokens"),
                func.coalesce(func.sum(source.c.cached_input_tokens), 0).label("cached_input_tokens"),
            )
            .select_from(source.outerjoin(ApiKey, ApiKey.id == source.c.api_key_id))
            .group_by(source.c.api_key_id, ApiKey.name, ApiKey.key_prefix)
            .order_by(func.coalesce(func.sum(source.c.input_tokens), 0).desc())
        )
        return [
    ...
    async def list_filter_options(
        ...
        account_ids: list[str] | None = None,
        api_key_ids: list[str] | None = None,
    ) -> tuple[list[str], list[str], list[ApiKeyReportOption]]:
        source = report_source(
        ...
        keys = (
            await self._session.execute(
                select(source.c.api_key_id, ApiKey.name, ApiKey.key_prefix)
                .select_from(source.outerjoin(ApiKey, ApiKey.id == source.c.api_key_id))
                .where(source.c.api_key_id.is_not(None))
                .distinct()
                .order_by(source.c.api_key_id)
            )
        ).all()
        return (
            sorted({row.model for row in pairs}),
            sorted(
                {
            ...
            [
                ApiKeyReportOption(api_key_id=row.api_key_id, api_key_name=row.name, key_prefix=row.key_prefix)
                for row in keys
            ],
        )
