...
def _prepare_websocket_request_state_for_account_switch(
    ...
    if replay_text is not None and request_state.prompt_cache_continuation_attempted:
        # Account-neutral fresh replay: the resend is self-contained again, so
        # leave the anchor-era stage behind and give back the reroute permission
        # withheld while the request was pinned to the anchor's account.
        request_state.request_stage = "first_turn"
        _restore_prompt_cache_continuation_soft_capacity_reroute(request_state)
    return replay_text
