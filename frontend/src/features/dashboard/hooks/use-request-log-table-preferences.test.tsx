...
describe("useRequestLogTablePreferences", () => {
  ...
  it("shows the default request-log columns initially", () => {
    const { result } = renderHook(() => useRequestLogTablePreferences());

    expect(result.current.visibleColumns).toEqual(DEFAULT_REQUEST_LOG_COLUMNS);
  ...
  it("restores the default columns, clears widths, and removes stored customization", () => {
    const { result } = renderHook(() => useRequestLogTablePreferences());
    act(() => {
      result.current.toggleColumn("plan");
