...
type ReportsSummaryFixture = Omit<
  ReportSummary,
  "totalCancelled" | "totalReasoningTokens" | "reasoningUsageKnownRequests" | "cacheHitRatio"
> & {
...
describe("ReportsSummaryCards", () => {
  it("renders cache usage semantics alongside cost and request comparisons", () => {
    ...
    expect(tokensCard.parentElement).toHaveClass("sm:grid-cols-2", "lg:grid-cols-3");
    expect(
  ...
  it("preserves trailing zeroes for unrelated whole K and B values", () => {
    ...
    const tokensCard = screen.getByTestId("report-summary-card-tokens");
    const inputTokensCard = screen.getByTestId("report-summary-card-input-tokens");
    const requestsCard = screen.getByTestId("report-summary-card-requests");
    ...
    expect(within(inputTokensCard).getByText("100.0B")).toBeInTheDocument();
    expect(within(inputTokensCard).getByText("Provider-reported logged usage")).toBeInTheDocument();
