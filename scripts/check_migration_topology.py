...
5. Branch fork against a base ref (error, skipped when the ref is absent): a
   ...
   head. An imported upstream lineage is accepted when the checkout contains an
   explicit merge revision joining that lineage to a base-ref head; the merge is
   the provenance that the second parent is an intentional release import rather
   than an unreviewed branch fork. This is the only check that sees the fork from the *branch alone*,
...
def check_branch_fork(revisions: Sequence[Revision], base_revisions: Sequence[Revision], base_ref: str) -> Report:
    ...
    imported_branch_revisions = _explicit_imported_branch_revisions(revisions, base_ids, base_heads)
    for revision in revisions:
        if revision.revision in base_ids:
            continue
        if revision.revision in imported_branch_revisions:
            continue
...
def _explicit_imported_branch_revisions(
    revisions: Sequence[Revision], base_ids: set[str], base_heads: Sequence[str]
) -> set[str]:
    """Return branch revisions joined to a base head by an explicit merge.

    Upstream release tags can continue from a revision that ``main`` has
    already advanced past. The release import is safe to accept when this
    checkout also contains a merge revision whose one parent is a current
    ``base_ref`` head and whose other parent is the imported branch. Walking
    only that second parent's lineage keeps the exception narrow: an ordinary
    single-parent branch still reports a fork, while the checked-in merge is
    the auditable provenance for an intentional convergence.
    """
    parents = {revision.revision: revision.down_revisions for revision in revisions}
    imported: set[str] = set()
    base_head_ids = set(base_heads)
    for revision in revisions:
        if len(revision.down_revisions) < 2:
            continue
        base_parents = [parent for parent in revision.down_revisions if parent in base_head_ids]
        branch_parents = [parent for parent in revision.down_revisions if parent not in base_ids]
        if len(base_parents) != 1 or len(branch_parents) != 1:
            continue
        pending = [branch_parents[0]]
        while pending:
            current = pending.pop()
            if current in imported or current in base_ids:
                continue
